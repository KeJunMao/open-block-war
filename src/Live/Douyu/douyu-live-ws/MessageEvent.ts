...
//     console.log(
//       "[开播提醒]",
//       r.ss == 1 ? config.MSG_LIVE_ON : config.MSG_LIVE_OFF
//     );
//   },
...
export default class MessageEvent {
  ...
  handle(data: ArrayBuffer) {
    Packet.Decode(data, (message: string) => {
      const response = STT.deserialize(message);
      this.live.emit("message", response);
      this.live.emit(response?.type, response);
    });
  }
}
